...
test('POST /products 201 (admin)', async () => {
  const { status, body } = await request(app())
    .post(`${apiRoot}`)
    .send({ access_token: adminSession, name: 'test', pictures: 'test', description: 'test', isShown: 'test', category: 'test', tags: 'test', stock: 'test', bajoPedido: 'test' })
  expect(status).toBe(201)
  expect(typeof body).toEqual('object')
  expect(body.name).toEqual('test')
  expect(body.pictures).toEqual('test')
  expect(body.description).toEqual('test')
  expect(body.isShown).toEqual('test')
  expect(body.category).toEqual('test')
  expect(body.tags).toEqual('test')
  expect(body.stock).toEqual('test')
  expect(body.bajoPedido).toEqual('test')
})
