...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your jewelshop-back account.<br>
        ...
        &mdash; jewelshop-back Team
      `
      return sendMail({ toEmail: email, subject: 'jewelshop-back - Password Reset', content })
    })
